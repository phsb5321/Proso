...
- [x] T6 — Built shared before server (`./scripts/generate-prisma.sh` +
  ...
  focused test 5/5 (4 original + 1 padded-trim plant added after the
  round-1 Codex BLOCK); FULL server suite with real PostgreSQL via
  testcontainers + Nix engine: **37/37 suites, 506/506 tests**
  (= 501 baseline + 5 new).
