...
export function registerAudioHandlers(registry: HandlerRegistry): void {
   ...
   */
  registry.register<unknown, Result<VoicesResponse, AudioHandlerError>>(
    'audio.getVoices',
    async (params) => {
      ...
      try {
        ...
        const result = await audioGenerator.getVoices(parsed.data.language);
   ...
   */
  registry.register<unknown, Result<SetVoiceResponse, AudioHandlerError>>(
    'audio.setVoice',
    async (params) => {
      ...
      try {
        ...
        await settingsStore.updateSettings({ voice: parsed.data.voiceId });
   ...
   */
  registry.register<unknown, Result<GenerateAudioResponse, AudioHandlerError>>(
    'audio.generate',
    async (params) => {
      ...
      try {
        ...
        const result = await audioGenerator.generateAudio(parsed.data as AudioRequest);
