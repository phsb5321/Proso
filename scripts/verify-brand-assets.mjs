...
const FONT_MANIFEST = resolve(ROOT, 'brand/fonts/manifest.json');
const FONT_FILE = resolve(ROOT, 'brand/fonts/OutfitVariable.ttf');
...
function assertBrandSvg(filename) {
  ...
  if (filename.includes('wordmark') || filename.includes('lockup')) {
    requireIds(filename, idSet, ['letter-p', 'letter-r', 'letter-o-1', 'letter-s', 'letter-o-2']);
  }
  if (filename === 'proso-wordmark.svg') {
    ...
    // Outline geometry is bound to the vendored font by
    // assertFontWordmarkSource (font hash + recorded outline hash); hand
    // substitution fails there before this structural check runs.
  }
}
...
function assertFontWordmarkSource() {
  let manifest;
  try {
    manifest = JSON.parse(readFileSync(FONT_MANIFEST, 'utf8'));
  } catch {
    fail('brand/fonts/manifest.json is missing or invalid');
  }
  if (manifest.font !== 'brand/fonts/OutfitVariable.ttf') {
    fail('wordmark font manifest path drifted');
  }
  const actualFontHash = sha256(readFileSync(FONT_FILE));
  if (actualFontHash !== manifest.fontSha256) {
    fail(`vendored wordmark font hash drifted: ${actualFontHash} != ${manifest.fontSha256}`);
  }
  if (
    typeof manifest.axes?.wght !== 'number' ||
    manifest.axes.wght < 400 ||
    manifest.axes.wght > 900
  ) {
    fail('wordmark font axes are not pinned');
  }
  if (typeof manifest.trackingPerMille !== 'number') {
    fail('wordmark font tracking is not pinned');
  }
  ...
  if (!source.includes('Generated by scripts/extract-font-wordmark.py')) {
    fail('wordmark source is not generated by the pinned font extraction script');
  }
  // The generation comment records the instance; a manifest edit without a
  // regeneration would otherwise be invisible to the outline hash.
  if (!source.includes(`wght=${manifest.axes.wght}`)) {
    fail(`wordmark source was not generated at the manifest weight ${manifest.axes.wght}`);
  }
  if (!source.includes(`tracking ${manifest.trackingPerMille}‰`)) {
    fail(
      `wordmark source was not generated at the manifest tracking ${manifest.trackingPerMille}‰`,
    );
  }
