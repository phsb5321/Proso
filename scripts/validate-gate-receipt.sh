...
RECEIPT_VERIFIED_AT="$(jq -r '.verifiedAt' "$RECEIPT_PATH")"
readonly RECEIPT_VERIFIED_AT
...
RECEIPT_BASE="$(jq -r '.baseRef' "$RECEIPT_PATH")"
readonly RECEIPT_BASE
RECEIPT_BASE_SHA="$(jq -r '.baseSha' "$RECEIPT_PATH")"
readonly RECEIPT_BASE_SHA
...
EXPECTED_BASE_SHA="$(git rev-parse "${RECEIPT_BASE}^{commit}")"
readonly EXPECTED_BASE_SHA
...
readonly EXPECTED_HEAD
EXPECTED_DIFF="$(sha256sum "$BUNDLE_PATH" | cut -d ' ' -f 1)"
readonly EXPECTED_DIFF
RECEIPT_HEAD="$(jq -r '.head' "$RECEIPT_PATH")"
readonly RECEIPT_HEAD
RECEIPT_DIFF="$(jq -r '.diffSha256' "$RECEIPT_PATH")"
readonly RECEIPT_DIFF
