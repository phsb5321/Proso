/**
 * LocalApplianceAudioAdapter Unit Tests
 *
 ...
 *
 * @module tests/unit/adapters/audio/local-appliance-audio.adapter
 */
...
import {
  ...
  parseWavDurationMs,
} from '../../../../src/adapters/audio/local-appliance-audio.adapter';
...
function makeAdapter(options: StubOptions = {}): {
  adapter: LocalApplianceAudioAdapter;
  fetchStub: FetchStub;
} {
  const fetchStub = makeFetchStub(options);
  return {
    adapter: new LocalApplianceAudioAdapter({ baseUrl: BASE_URL, fetchFn: fetchStub }),
    fetchStub,
  };
}
...
interface GenerateOutcome {
  ...
  readonly adapter: LocalApplianceAudioAdapter;
}
...
runAudioGeneratorContractTests('LocalApplianceAudioAdapter', () => makeAdapter().adapter);

describe('LocalApplianceAudioAdapter', () => {
  describe('POST /v1/tts request shape', () => {
    ...
    it('derives a different key when input, voice or speed differ', async () => {
      const keys = await Promise.all([
        deriveIdempotencyKey('texto', 'voice-a', 1),
        deriveIdempotencyKey('outro', 'voice-a', 1),
        deriveIdempotencyKey('texto', 'voice-b', 1),
        deriveIdempotencyKey('texto', 'voice-a', 1.5),
      ]);

      const values = keys.flatMap((key) => (isOk(key) ? [key.value] : []));
  ...
  describe('successful synthesis', () => {
    ...
    it('reports a non-WAV body as a provider error instead of guessing a duration', async () => {
      ...
      expectProviderErrorCode(result, APPLIANCE_ERROR_CODES.invalidResponse);
    });
  });
  ...
  describe('abort handling', () => {
    ...
    it('returns an abort error without issuing a request when already aborted', async () => {
      ...
      expectProviderErrorCode(result, APPLIANCE_ERROR_CODES.aborted);
      expect(fetchStub.calls).toHaveLength(0);
    });
  ...
  describe('transport failures', () => {
    it('distinguishes a DNS failure from a generic network failure', async () => {
      ...
      expectProviderErrorCode(result, APPLIANCE_ERROR_CODES.dnsFailure);
    });
  ...
  describe('voice resolution (D-2)', () => {
    ...
    it('declines a requested voice the appliance does not publish', async () => {
      ...
      expectProviderErrorCode(result, APPLIANCE_ERROR_CODES.noVoice);
      expectNoTtsRequest(fetchStub);
    });
  });
  ...
  describe('base URL handling', () => {
    it('tolerates a trailing slash in the configured base URL', async () => {
      ...
      const adapter = new LocalApplianceAudioAdapter({
        baseUrl: `${BASE_URL}/`,
        fetchFn: fetchStub,
      });
  ...
  describe('idempotency key derivation', () => {
    ...
    it('reports a typed error rather than hand-rolling a hash when subtle is missing', async () => {
      ...
      const result = await deriveIdempotencyKey('texto', 'voice-a', 1);

      Object.defineProperty(globalThis.crypto, 'subtle', { value: subtle, configurable: true });

      expect(isErr(result)).toBe(true);
      if (!isErr(result) || result.error.type !== 'provider_error') return;
      expect(result.error.code).toBe(APPLIANCE_ERROR_CODES.cryptoUnavailable);
    });
  });
});
