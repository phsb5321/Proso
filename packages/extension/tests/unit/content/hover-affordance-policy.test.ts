...
describe('shouldOfferHoverAffordance', () => {
  ...
  it('does not treat an extension popup as a page origin or transfer engagement to another origin', () => {
    ...
    expect(
      shouldOfferHoverAffordance({ origin: 'https://first-visit.example', engagedOrigins }),
    ).toBe(false);
    ...
    expect(shouldOfferHoverAffordance({ origin: popupOrigin, engagedOrigins: [popupOrigin] })).toBe(
      false,
    );
  });

  it.each([undefined, null, '', 'null', 'file://', 'not a url', `${ORIGIN}/page`])(
    'rejects missing or invalid origin %s even if present in storage',
    (origin) => {
      expect(shouldOfferHoverAffordance({ origin, engagedOrigins: [origin] })).toBe(false);
    },
  );

  it.each([undefined, null, ORIGIN, {}, 42])(
    'fails closed on malformed storage %s',
    (engagedOrigins) => {
      expect(shouldOfferHoverAffordance({ origin: ORIGIN, engagedOrigins })).toBe(false);
    },
  );
...
describe('recordEngagedOrigin', () => {
  ...
  it.each([undefined, null, '', 'null', 'not a url'])(
    'does not record absent origin %s',
    (origin) => {
  ...
  it('recovers from malformed storage and removes invalid entries', () => {
    ...
    expect(recordEngagedOrigin([null, 3, 'null', `${ORIGIN}/path`, ORIGIN], ORIGIN)).toEqual([
      ORIGIN,
    ]);
  });
});
