...
describe('franc-min Language Detection Accuracy', () => {
  ...
  it('should achieve ≥80% accuracy (16/20 correct detections)', () => {
    ...
    results.forEach((r) => {
      ...
      console.log(`  ${status} Sample ${r.index + 1}: expected=${r.expected}, detected=${r.detected} (raw=${r.raw})`);
    });
    ...
    if (failures.length > 0) {
      ...
      failures.forEach((f) => {
        console.log(`  - ${f.expected.toUpperCase()}: Detected as ${f.detected} (sample ${f.index + 1})`);
      });
    }
...
describe('franc-min Initialization Performance', () => {
  it('should initialize and detect language in <50ms (T097)', () => {
    const startTime = performance.now();

    // First call - includes module warm-up
    const result1 = franc('This is a test of the language detection system.', { minLength: 10 });

    const firstCallTime = performance.now() - startTime;
    console.log(`First detection call: ${firstCallTime.toFixed(2)}ms`);

    // Subsequent calls should be faster
    const start2 = performance.now();
    for (let i = 0; i < 10; i++) {
      franc('Another test sentence for language detection performance.', { minLength: 10 });
    }
    const avgTime = (performance.now() - start2) / 10;
    console.log(`Average detection time (10 calls): ${avgTime.toFixed(2)}ms`);

    // First call should be under 50ms (the hard gate from tasks.md)
    expect(firstCallTime).toBeLessThan(50);
