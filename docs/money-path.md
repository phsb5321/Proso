...
| ✓ | The deploy-readiness receipt never prints an unproven claim endpoint CLOSED: while purchase is disabled the hold is NOT REQUIRED (exit 0, no probe sent); a complete config binds CLOSED only to a registered route that answered the canonical 202, anything else stays OPEN and exits 1 | PROSO-40 fix; falsifier matrix in `checkout-surface-gate.mjs` (`the deploy receipt fails closed and its oracles are runnable, not greppable`) |
...
5. **Backend probe gate** — with the backend deployed, probe the claim
   ...
   NOT REQUIRED and no probe is even sent — the live-202 hold binds only
   when the config is complete (`checkout-deploy-readiness.mjs:249,258`).
